...
export class MCPDocsServer {
        ...
        tools: [
          {
            name: "list_files",
            description:
              "List all documentation files in the specified directory. " +
          ...
          },
          {
            ...
            description:
              "Update an existing document with new content and/or metadata. " +
          ...
          },
          {
            ...
            description:
              "Search documents for the specified query. " +
          ...
          },
          {
            name: "analyze_docs",
            description:
              "Analyze documentation health in the specified directory. " +
          ...
          },
          {
            ...
            description:
              "Get overall documentation health score. " +
   ...
   */
  public async executeHandler(name: string, args: any): Promise<string> {
    switch (name) {
      case "list_files":
  ...
  private async ensureDocsDir() {
    try {
      ...
      if (this.options.createIfNotExists) {
        ...
        throw new Error(
          `Docs directory does not exist: ${this.docsDir}. Use --create-dir to create it.`
        );
      }
    }
  }
   ...
   */
  private async getAllDirectories(
    dir: string,
    basePath: string = ""
  ): Promise<string[]> {
    const entries = await fs.readdir(dir, { withFileTypes: true });
   ...
   */
  private async readDocument(docPath: string): Promise<DocumentWithMetadata> {
   ...
   * @returns A list of directories
   */
  private async handleListDirectories(
    args: z.infer<typeof ListDirectoriesArgsSchema>
  ): Promise<string> {
    const { directory } = args;
    const dirPath = this.getAbsolutePath(directory);
  ...
  private async handleGetDocument(
    args: z.infer<typeof GetDocumentArgsSchema>
  ): Promise<string> {
  ...
  private async handleCreateDocument(
    args: z.infer<typeof CreateDocumentArgsSchema>
  ): Promise<string> {
  ...
  private async handleUpdateDocument(
    args: z.infer<typeof UpdateDocumentArgsSchema>
  ): Promise<string> {
    const { path: docPath, content, metadata } = args;
    const fullPath = this.getAbsolutePath(docPath);
    ...
    const { content: existingDocContent, metadata: existingMetadata } =
      this.parseDocument(existingContent);
  ...
  private async handleDeleteDocument(
    args: z.infer<typeof DeleteDocumentArgsSchema>
  ): Promise<string> {
    const { path: docPath } = args;
    const fullPath = this.getAbsolutePath(docPath);
  ...
  private async handleSearchDocuments(
    args: z.infer<typeof SearchDocumentsArgsSchema>
  ): Promise<string> {
    ...
    const matchingDocuments = documents.filter((doc) => {
      ...
      // Match by metadata
      const metadataMatch = Object.entries(doc.metadata).some(
        ([key, value]) => {
   ...
   */
  private async handleAnalyzeDocs(
    args: z.infer<typeof AnalyzeDocsArgsSchema>
  ): Promise<string> {
    const { directory } = args;
    const documents = await this.getAllDocuments(directory);
    ...
    const documentsWithTags = documents.filter(
      (doc) => Array.isArray(doc.metadata.tags) && doc.metadata.tags.length > 0
    ).length;

    const documentsWithStatus = documents.filter(
      (doc) => doc.metadata.status !== undefined
    ).length;
    ...
    const shortDocuments = documents.filter(
      (doc) => doc.content.trim().length < 100
    );

    // Check for missing titles
    const missingTitles = documents.filter(
      (doc) => !doc.content.match(/^#\s.+/m) && !doc.metadata.title
    );
    ...
    return JSON.stringify(
      {
        ...
        statistics: {
          ...
          tagsPercentage: Math.round(
            (documentsWithTags / totalDocuments) * 100
          ),
          statusPercentage: Math.round(
            (documentsWithStatus / totalDocuments) * 100
          ),
        },
        issues: {
   ...
   */
  private async handleGetHealthScore(
    args: z.infer<typeof GetHealthScoreArgsSchema>
  ): Promise<string> {
    ...
    const totalScore = Math.round(
      tagsScore + statusScore + lengthScore + titleScore
    );
  ...
  private async handleGetSuggestions(
    args: z.infer<typeof GetSuggestionsArgsSchema>
  ): Promise<string> {
    const documents = await this.getAllDocuments();
    ...
    const documentsWithoutTags = documents.filter(
      (doc) =>
        !Array.isArray(doc.metadata.tags) || doc.metadata.tags.length === 0
    );
    if (documentsWithoutTags.length > 0) {
      suggestions.push(
        `- Add tags to ${documentsWithoutTags.length} documents without tags`
      );
    }

    // Check for documents without status
    const documentsWithoutStatus = documents.filter(
      (doc) => doc.metadata.status === undefined
    );
    if (documentsWithoutStatus.length > 0) {
      suggestions.push(
        `- Add status to ${documentsWithoutStatus.length} documents without status`
      );
    }
    ...
    if (shortDocuments.length > 0) {
      suggestions.push(
        `- Expand ${shortDocuments.length} short documents that have less than 100 characters`
      );
    }
    ...
    if (missingTitles.length > 0) {
      suggestions.push(
        `- Add titles to ${missingTitles.length} documents without titles`
      );
    }

    // General suggestions
    suggestions.push(
      "- Consider organizing documentation in a logical directory structure",
      "- Add a README.md file to each directory to explain its contents",
      "- Use consistent formatting and style throughout documentation",
      "- Add links between related documents to improve navigation"
    );
